...
(async () => {
  ...
  const storedHash = db2.prepare('SELECT recovery_token_hash FROM local_credentials WHERE user_id = ?').get(ownerRow.id).recovery_token_hash;
  ...
  const afterExpired = db2.prepare('SELECT recovery_token_hash, recovery_token_expires_at FROM local_credentials WHERE user_id = ?').get(ownerRow.id);
  assertEq(afterExpired.recovery_token_hash, null, 'expired consume clears hash');
  assertEq(afterExpired.recovery_token_expires_at, null, 'expired consume clears expiry');
  ...
  const afterRevoke = db2.prepare('SELECT recovery_token_hash FROM local_credentials WHERE user_id = ?').get(ownerRow.id).recovery_token_hash;
  ...
  assert(!/password_hash|recovery_token_hash/.test(pathsJson), 'login-paths response carries no secret columns');
  ...
  console.log('\nGroup 5 — scripts/reset-owner-password.js CLI');
  ...
  const cliOut = execFileSync('node', [
    path.resolve(__dirname, '..', 'scripts', 'reset-owner-password.js'),
  ], {
  ...
  assertEq(cliMeta.source, 'reset-owner-password.js', 'CLI audit meta.source is the script path');
