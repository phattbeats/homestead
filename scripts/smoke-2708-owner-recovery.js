  ...
  try {
    ...
    const ownerLocal = db.prepare(`
      SELECT user_id, password_hash IS NOT NULL AS has_password,
             owner_recovery_token_hash AS rt_hash_present,
             owner_recovery_token_expires_at AS rt_expires_at,
    ...
    fs.writeFileSync(path.join(verifyOut, 'smoke-2708-db-shape.json'), JSON.stringify({
      owner_local_credential: ownerLocal ? {
        ...
        owner_recovery_token_hash_present: !!ownerLocal.rt_hash_present,
        owner_recovery_token_expires_at: ownerLocal.rt_expires_at,
      } : null,
    ...
    const cliRaw = execFileSync('node', [
      path.resolve(__dirname, 'owner-recovery.js'),
    ], {
    ...
    const storedRows = db2.prepare(`SELECT owner_recovery_token_hash, owner_recovery_token_expires_at FROM local_credentials WHERE user_id = (SELECT id FROM users WHERE is_admin = 1 LIMIT 1)`).get();
    if (storedRows.owner_recovery_token_hash !== null) throw new Error('owner_recovery_token_hash must be NULL after consume + revoke');
    if (storedRows.owner_recovery_token_expires_at !== null) throw new Error('owner_recovery_token_expires_at must be NULL after consume + revoke');
    db2.close();
