...
// dedicated `owner_recovery_token_hash` / `owner_recovery_token_expires_at`
// columns — never the shared `recovery_token_hash` /
// `recovery_token_expires_at` columns that `reset-owner-password.js`
// owns. Don't merge the two; they solve different problems and
// colliding their storage would let minting one silently invalidate
// the other for the same user.
//
// Usage:
//   DATA_DIR=/var/lib/homestead node scripts/owner-recovery.js
//   DATA_DIR=/var/lib/homestead node scripts/owner-recovery.js --ttl-min 30
//   DATA_DIR=/var/lib/homestead node scripts/owner-recovery.js --revoke
//
// What it prints:
...
try {
  ...
  if (revokeMode) {
    ...
    identity.auditOwnerRecovery(db, {
      ...
      meta: { source: 'owner-recovery.js', mode: 'revoke' },
    });
  ...
  identity.auditOwnerRecovery(db, {
    ...
    meta: { source: 'owner-recovery.js', ttl_min: ttlMin },
  });
