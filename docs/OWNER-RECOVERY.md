...
`owner_recovery_token_expires_at` columns so the two mechanisms never
collide.
...
`scripts/owner-recovery.js` is your safest available
...
DATA_DIR=/var/lib/homestead /usr/local/bin/node \
  /opt/homestead/scripts/owner-recovery.js
```
...
the CLI, and `meta.source = 'owner-recovery.js'`. You can
...
```bash
DATA_DIR=/var/lib/homestead /usr/local/bin/node \
  /opt/homestead/scripts/owner-recovery.js --ttl-min 10
```
...
```bash
DATA_DIR=/var/lib/homestead /usr/local/bin/node \
  /opt/homestead/scripts/owner-recovery.js --revoke
```
...
| `mint` stores `owner_recovery_token_hash = sha256(token)`; the plaintext never touches the DB. | `Group 2` |
