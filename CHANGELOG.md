...
## v0.1.15 (2026-08-11) — PHA-1902 (PHA-1617.9)
...
- **Tests**: 50 + 31 = 81 new assertions (`scripts/test-snapshot.js`
  + `scripts/smoke-snapshot.js`), wired into `npm test` and
  `npm run test:smoke`. The smoke exercises the end-to-end flow
  against a live `server.js`: auth, scoping, today/upcoming windows,
  done-vs-open filtering, and the `X-Homestead-Tz` override.
...
## v0.1.14 (2026-08-11) — PHA-1907

- **Fix: include `lib/` in the runtime image.** `server.js` requires
  `./lib/user-model`, `./lib/agent-tokens`, `./lib/sync/*`, and
  `./lib/dedup/matcher`, but the Dockerfile copied `node_modules`,
  `package.json`, `server.js`, and `public/` and never `lib/`. The
  container crashed on first `require` with `Cannot find module
  ./lib/user-model` (added v0.0.5 by PHA-1618). Adds `COPY lib ./lib`
  to the runtime stage. Operator pulled `v0.1.14` on PHATT-RAID to
  restore the calendar/agent-tokens/PAT surface.
