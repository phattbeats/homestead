...
async function checkOne(db, service, hooks, opts) {
  ...
  // PHA-2210: tile_health_transition event (UP <-> DOWN). Fires on every
  // transition (both directions) so the operator dashboard can show the
  // recovery histogram. Best-effort; never throws into the health probe.
  if (state.transitioned) {
    analytics.logEvent(db, {
      userId: null,                  // system-initiated; no user
