...
const SCOPE_PHRASES = {
  ...
  'read:walls:household': 'Read posts on the household wall',
  ...
  // PHA-2851: Hearth's house-actions. One scope per action rather than a
  // blanket `write:actions` — "queue things onto my watch-next list" and
  // "message other people in my name" are not the same consent, and an
  // app that needs the first should never be handed the second.
  'write:actions:media_queue': 'Queue movies, shows and books onto your watch-next list',
  'write:actions:mention': 'Message other household members on your behalf',
  ...
  // PHA-2426: agent-to-agent mailbox. An installed app holding these is
  // itself a foreign household's harness, not a household member — see
  // lib/porch/mailbox.js's header comment for the untrusted-data and
  // Porch-visibility rules that gate what these scopes are allowed to do.
  'read:mailbox': "Read its own thread's mailbox messages",
  'write:mailbox': 'Send mailbox messages, mirrored on the household Porch',
