...
//
// Returns an object with listCalendars() and listEvents(range).
// Phase-2 write methods are stubs that throw — the work order defers
// them to a follow-up.
function makeCalDAVSource(config, deps = {}) {
  ...
  return {
    ...
    listCalendars,
    listEvents,
    // Phase 2 stubs — see work order step 4.
    createEvent: async () => { throw new Error('CalDAVSource.createEvent: Phase 2, not implemented'); },
    updateEvent: async () => { throw new Error('CalDAVSource.updateEvent: Phase 2, not implemented'); },
    deleteEvent: async () => { throw new Error('CalDAVSource.deleteEvent: Phase 2, not implemented'); },
  };
}
...
module.exports = { makeCalDAVSource, parseVEvents, parseICalDate, _internals: { xml2jsLite, formatIcsDate, defaultHttpDo } };
