...
//   await adapter.listCalendars() -> [{ href, displayName }]
...
const { makeGoogleSource } = require('./google-source');
...
// PHA-1865: Google Calendar adapter. The decrypted cred_blob for
// provider='google' must contain { access_token, refresh_token,
// expires_at, client_id, client_secret?, scope? } — see
// lib/google-source.js for the refresh-on-401 contract.
//
// This branch keeps the PHA-1865 Google adapter scoped to just the
// Google additions; the PHA-1864 Graph (ms365) adapter is the parallel
// PR #7. Either PR can merge first against the shared base
// (pha-1620-calendar-adapters); both register their adapter behind
// the same `registerAdapter` call shape so the merged main looks
// identical regardless of merge order.
registerAdapter('google', (config, deps) => makeGoogleSource(config, deps));
