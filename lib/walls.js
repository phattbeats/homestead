...
// least one live agent_tokens row (lib/agent-tokens.js, PHA-1617).
// require order matters in server.js (agent-tokens migrates before
// walls is ever queried), but plenty of narrower test/smoke scripts
// exercise walls.js without agent-tokens.migrate() — no agent_tokens
// table there just means zero agents exist yet, not an error.
...
// identifies agents — a live agent_tokens row — not a stored flag.
function userView(userId) {
  const u = _db.prepare('SELECT id, username, display FROM users WHERE id = ?').get(userId);
  return u ? { username: u.username, display: u.display, isAgent: isAgentUserId(u.id) } : null;
}
