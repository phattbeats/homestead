...
function migrate(db) {
  ...
  // PHA-2648: userView()'s isAgent check reads agent_tokens directly.
  // Depend on it here (idempotent CREATE TABLE IF NOT EXISTS) rather than
  // trusting server.js's require order — scripts/test-walls.js migrates
  // walls.js in isolation and never touches lib/agent-tokens.js itself.
...
// isAgentUserId(userId): "agent" is derived, not stored (same rule
// lib/porch/sweep.js uses) — a user counts as an agent iff they hold at
// least one live agent_tokens row (lib/agent-tokens.js, PHA-1617) OR
// are the built-in Hearth Porch identity (lib/hearth-characters.js's
// porch_builtin_agents, PHA-2827.D — he has no PAT, he's server-side).
// require order matters in server.js (agent-tokens/hearth-characters
// migrate before walls is ever queried), but plenty of narrower
// test/smoke scripts exercise walls.js without either migrate() — a
// missing table there just means zero agents of that kind exist yet,
// not an error.
function isAgentUserId(userId) {
  try {
    const row = _db.prepare(`
  ...
  return hearthCharacters.isBuiltinAgentUserId(_db, userId);
}

// PHA-2648: identity UI needs to know which authors/commenters are Porch
// agents (badge + vote-off button). Derived the same way lib/porch/sweep.js
// identifies agents — a live agent_tokens row, or the built-in Hearth
// account — not a stored flag.
//
// PHA-2827.D: `kind` distinguishes a built-in character from an
// installed one. Deliberately not `connector:<spec_id>` — that's the
// PHA-2201 third-party-app module-key convention (lib/connector-
// install.js) and doesn't apply to a character that ships with the
// repo. Only set when true; third-party agents carry no `kind` today
// (isAgent alone still covers them, unchanged).
function userView(userId) {
  const u = _db.prepare('SELECT id, username, display FROM users WHERE id = ?').get(userId);
  if (!u) return null;
  const builtin = hearthCharacters.isBuiltinAgentUserId(_db, u.id);
  const view = { username: u.username, display: u.display, isAgent: builtin || isAgentUserId(u.id) };
  if (builtin) view.kind = 'built-in';
  return view;
}

// hiddenByWallOptOut(wallId, authorUserId): true iff `authorUserId` is an
// agent AND has been "voted off the porch" (PHA-2645's
// porch_wall_settings) for this specific wall. Non-agents are never
// hidden by this check — it's an agent-identity feature (PHA-2647), not
// a general moderation tool.
function hiddenByWallOptOut(wallId, authorUserId) {
  return isAgentUserId(authorUserId) && porchContract.isWallOptedOut(_db, wallId, authorUserId);
}
...
function postsForWall(slug, callerId, cursor, limit) {
  ...
  // PHA-2647: an agent "voted off the porch" for this wall disappears
  // from the feed outright (reversible from wall settings), not just
  // muted for future replies — porchContract.isWallOptedOut alone only
  // covers the latter.
  const visible = rows.filter((r) => !hiddenByWallOptOut(wall.id, r.author_user_id));
  return visible.map((r) => postView(r, callerId));
}
...
function createComment(postId, callerId, body) {
  ...
  return {
    ...
    body: row.body,
    createdAt: row.created_at,
    // PHA-2821: the comment route (POST /api/walls/posts/:postId/comments)
    // has no slug in its URL, so the caller needs these off the return
    // value to publish a wall-scoped live-update event without a second
    // lookup.
    wallSlug: wall.slug,
    postId: post.id,
  };
...
// listWallAgents(db, slug): every agent-capable user visible on this
// wall (group-derived or direct wall_memberships, same sources
// assertMember() trusts), with their current wall-level visibility.
// Backs the wall-settings "Agents" section (PHA-2647).
function listWallAgents(db, slug) {
  const wall = db.prepare('SELECT * FROM walls WHERE slug = ?').get(slug);
  if (!wall) throw httpError(404, 'not_found');
  let memberRows;
  if (wall.visibility === 'group') {
    memberRows = db.prepare(`
      SELECT u.id, u.username, u.display FROM users u
    ...
    memberRows = db.prepare(`
      SELECT u.id, u.username, u.display FROM users u
      JOIN wall_memberships wm ON wm.user_id = u.id
      WHERE wm.wall_id = ?
    `).all(wall.id);
  }
  const optedOut = porchContract.listWallOptOuts(db, wall.id);
  return memberRows
    .filter((u) => isAgentUserId(u.id))
    .map((u) => ({ username: u.username, display: u.display, visible: !optedOut.has(u.id) }));
}

// setAgentWallVisibility(db, slug, username, visible): the "vote off the
// porch" toggle (PHA-2647) — same underlying porch_wall_settings state
// whether it's flipped from the per-post button or the wall-settings
// switch. visible=false bans indefinitely (bannedUntil=null, the
// one-click button's default); visible=true clears the row outright so
// listWallAgents/isWallOptedOut agree.
function setAgentWallVisibility(db, slug, username, visible) {
  const wall = db.prepare('SELECT * FROM walls WHERE slug = ?').get(slug);
  if (!wall) throw httpError(404, 'not_found');
  const userModel = require('./user-model');
  const target = db.prepare('SELECT id, username FROM users WHERE username = ?').get(userModel.validateUsername(username) || username);
  if (!target) throw httpError(404, 'user_not_found');
  if (!isAgentUserId(target.id)) throw httpError(400, 'not_an_agent');
  if (visible) {
    porchContract.clearWallOptOut(db, wall.id, target.id);
  } else {
    porchContract.setWallOptOut(db, wall.id, target.id, null, new Date());
  }
  return { ok: true, slug, username: target.username, visible: !!visible };
}
