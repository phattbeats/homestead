...
function createPost(slug, callerId, body) {
  ...
  // PHA-2210: dual-write — notification_log (user-facing activity feed)
  // AND analytics_events (operator dashboard source). Atomic from the
  // caller's perspective; both writes succeed or both are skipped.
  analytics.logMutation(_db, {
    userId: callerId,
    notification: {
      category: 'wall',
      title: `New post in ${wall.name || wall.slug}`,
      body: kind === 'link' ? (body.link_title || body.link_url || '') : (body.text_body || `[${kind}]`),
      url: `/w/${wall.slug}`,
      tag: 'wall_post',
    },
    analytics: {
      ...
      meta: {
        ...
        post_kind: kind,
        has_media: !!body.media_id,
      },
    },
  });

  // PHA-2210: first_post funnel signal (one row per user ever).
  analytics.logFirst(_db, {
    userId: callerId,
    kind: 'first_post',
    subjectType: 'user',
    subjectId: callerId,
...
function toggleReaction(slug, postId, callerId, emoji) {
  ...
  // PHA-2210: analytics_events capture (toggle-ON only — toggling OFF
  // isn't an engagement signal, just a state correction).
  analytics.logEvent(_db, {
    userId: callerId,
    kind: 'wall_reaction_added',
    subjectType: 'post_reaction',
    subjectId: post.id,
    meta: { post_id: post.id, emoji },
  });

  // PHA-2210: first_reaction funnel signal (one row per user ever).
  analytics.logFirst(_db, {
    userId: callerId,
    kind: 'first_reaction',
    subjectType: 'user',
...
function createComment(postId, callerId, body) {
  ...
  // PHA-2210: dual-write (notification + analytics) for wall_comment_added.
  analytics.logMutation(_db, {
    userId: callerId,
    notification: {
      category: 'wall',
      title: `New comment on a post in ${wall.name || wall.slug}`,
      body: trimmed.slice(0, 200),
      url: `/w/${wall.slug}#post-${post.id}`,
      tag: 'wall_comment',
    },
    analytics: {
