...
function createPost(slug, callerId, body) {
  ...
  emitActivity(wall, row, callerId);
...
// emitActivity(wall, post, authorId): one notification_log row per
// recipient, keyed to the post's own created_at so the activity feed and
// the wall stay in the same chronological order (constitutional rule —
// ORDER BY created_at only, no scoring). Runs inline after the post insert.
function emitActivity(wall, post, authorId) {
  const author = userView(authorId);
  const title = `${(author && author.display) || 'Someone'} posted on ${wall.name}`;
  const body = post.kind === 'text' ? (post.text_body || '').slice(0, 200) : `[${post.kind}]`;
  const url = `/porch.html?wall=${encodeURIComponent(wall.slug)}&post=${encodeURIComponent(post.id)}`;
  const tag = `wall_post:${wall.slug}:${post.id}`;

  const recipients = recipientsFor(wall, authorId);
  const insert = _db.prepare(`
    INSERT INTO notification_log (user_id, category, title, body, url, tag, delivered, created_at)
    VALUES (?, 'wall_post', ?, ?, ?, ?, 1, ?)
  `);
  for (const userId of recipients) {
    insert.run(userId, title, body, url, tag, post.created_at);
  }
}
