...
// canDeletePost(wall, post, callerId): mirrors the gate inside deletePost()
// so the client can hide the affordance honestly (PHA-2657). Same rule on
// both sides — author OR wall-admin — keeps the UI accurate and the
// endpoint is the actual enforcement. Resolves admin via a single member
// lookup; non-members fall through to false (and the DELETE endpoint will
// 404 them on the membership check anyway).
function canDeletePost(wall, post, callerId) {
  if (!callerId) return false;
  if (post.author_user_id === callerId) return true;
  const m = _db.prepare('SELECT role FROM wall_memberships WHERE wall_id = ? AND user_id = ?').get(wall.id, callerId);
  return !!(m && m.role === 'admin');
}

function postView(row, userId, wall) {
  return {
    id: row.id,
    ...
    // PHA-2657: server-side gate mirroring walls.deletePost() so the UI
    // can hide the affordance honestly. The DELETE endpoint re-enforces
    // the rule, so a stale client flag can never authorize an action.
    canDelete: canDeletePost(wall, row, userId),
  };
}
...
function postsForWall(slug, callerId, cursor, limit) {
  const member = assertMember(slug, callerId);
  ...
  return rows.map((r) => postView(r, callerId, wall));
}
...
function createPost(slug, callerId, body) {
  ...
  return postView(row, callerId, wall);
}
