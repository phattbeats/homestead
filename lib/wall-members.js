...
//
// PHA-2556: group walls derive access from user_groups → groups.name
// (assertMember), so getMembers has to UNION in those users too —
// otherwise the member listing would only show people with explicit
// wall_memberships rows, which is empty for a freshly seeded group
// wall. The role column is wall_memberships.role when present, else
// 'member' (group-derived grants don't carry a wall_admin role).
function getMembers(db, wallSlug) {
  const wall = db.prepare('SELECT id, slug, name, visibility, group_name FROM walls WHERE slug = ?').get(wallSlug);
  if (!wall) return null;
  if (wall.visibility === 'group') {
    const rows = db.prepare(`
      SELECT u.id, u.username, u.display, u.color, u.avatar_url,
             COALESCE(wm.role, 'member') AS role,
             COALESCE(wm.joined_at, ug.granted_at, datetime('now')) AS joined_at
      FROM users u
      JOIN user_groups ug ON ug.user_id = u.id
      JOIN groups g ON g.id = ug.group_id
      LEFT JOIN wall_memberships wm ON wm.wall_id = ? AND wm.user_id = u.id
      WHERE g.name = ?
      ORDER BY joined_at ASC, u.username COLLATE NOCASE ASC
    `).all(wall.id, wall.group_name);
