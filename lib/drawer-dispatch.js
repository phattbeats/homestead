  ...
  if (agentRuntime && hearthCharacters) {
    ...
    if (character && character.character_key === agentRuntime.CHAR_KEY_HEARTH) {
      ...
      const hearthResult = await agentRuntime.dispatchHearth({
        ...
        // PHA-2851: Hearth's house-actions. Undefined means "the real
        // surface, scoped to this user"; tests pass an injected
        // { specs, execute } or `false` for the text-only path.
      ...
      if (hearthResult.ok) {
        ...
        return {
          ...
          attempts: 1,
          hearth: true,
          // PHA-2851: what Hearth actually did, for the drawer's
          // tool-result chips. Empty array on a text-only reply.
          toolResults: hearthResult.toolResults || [],
        };
      }
      ...
      return {
        ...
        attempts: 0,
        hearth: true,
        // PHA-2851: actions that already committed before the provider
        // fell over. Surfaced on the failure path too — a queue row
        // exists whether or not Hearth got to tell you about it.
        toolResults: hearthResult.toolResults || [],
      };
    }
  }
