...
const REGISTRY = Object.freeze({
  ...
  // `agentDrawer`.
  // PHA-2853 rework: the typed daily issue (Rotation Desk, Arts & Media,
  // The Porch, Today's Listings) reads wall activity and merged calendar
  // events directly, not just through the agent's prose layer — those are
  // now hard data dependencies, not optional context. `agent` stays too:
  // the LLM-authored lede/briefs still route through the BYOK harness.
  gazette: {
    key: 'gazette',
    name: 'Gazette',
