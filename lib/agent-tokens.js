...
function issue(db, userId, { label, expiresAt = null, scopes = 'user', createdByRunId = null } = {}) {
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    const r = db.prepare(`INSERT INTO agent_tokens
        (user_id, label, token_hash, token_prefix, scopes, expires_at, created_by_run_id)
        VALUES (?, ?, ?, ?, ?, ?, ?)`)
      .run(userId, clean, hash, prefix, scopes, expiresAt || null, createdByRunId);
