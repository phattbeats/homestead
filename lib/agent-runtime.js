...
// ---------------------------------------------------------------------------
//
// `composeGazette({ system, user, byokKey, providerCfg })` runs ONE
// non-streaming completion and returns its raw text. Same provider
// path, same BYOK-first key resolution, and the same
// `{ ok, status, lastError }` result contract as `draftPorchCandidate`
// above — the Gazette deliberately does not open a second LLM
// integration route.
//
// Two deviations from `draftPorchCandidate`, both because an edition is
// a bigger artifact than a one-line reaction:
//   * `maxTokens` defaults to 1500, not 200.
//   * The caller supplies the whole system prompt (lib/gazette.js owns
//     the voice rules and the JSON contract), so the Hearth character
//     prompt is NOT prepended. An edition is a household bulletin, not
//     Hearth speaking in character; VOICE.md is the register, and
//     lib/gazette.js hands those rules over directly.
async function composeGazette(opts) {
  const {
    system,
    user,
    byokKey = '',
  ...
  if (!system || !user) {
    return { ok: false, status: 'bad_request', lastError: 'system and user prompts are required' };
  }
  ...
  let firstError = null;
  try {
    for await (const ev of providerObj.stream([{ role: 'user', content: user }])) {
      if (ev.type === 'text') {
        fullText += ev.text;
      } else if (ev.type === 'error') {
        firstError = firstError || ev;
