...
`);
  ...
  if (userCount === 0) {
    // A public deployment with the historical fallback makes its admin
    // account trivially reachable. Fail before writing any user rows so a
    // first boot cannot leave behind a database that needs manual repair.
    const adminPassword = process.env.ADMIN_PASSWORD;
    if (!adminPassword || adminPassword === 'changeme') {
      throw new Error(
        'ADMIN_PASSWORD must be set to a non-default value before the first Homestead boot. ' +
        'Refusing to seed a publicly guessable admin account.'
      );
    }
    const ins = db.prepare('INSERT INTO users (username, display, color, pass_hash, is_admin) VALUES (?,?,?,?,0)');
    ins.run('admin', 'Admin', '#7c9a72', bcrypt.hashSync(adminPassword, 10));
    ins.run('brandon', 'Brandon', '#8a9ec4', bcrypt.hashSync(process.env.BRANDON_PASSWORD || adminPassword, 10));
    ins.run('emily', 'Emily', '#c48a9e', bcrypt.hashSync(process.env.EMILY_PASSWORD || adminPassword, 10));
  ...
  // Do not silently keep serving an old database whose privileged account
  // still accepts the historical documented default. bcrypt comparison is
  // required because each hash has a unique salt.
  const admin = db.prepare('SELECT pass_hash FROM users WHERE username = ?').get('admin');
  if (admin && admin.pass_hash && bcrypt.compareSync('changeme', admin.pass_hash)) {
    throw new Error(
      'The admin account still uses the insecure default password. Rotate it in the database ' +
      'before restarting Homestead; refusing to serve the application.'
    );
  }
