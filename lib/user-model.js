...
//
// PHA-2704 (identity foundation): after creating the legacy `users`
// shape and backfilling its v0.0.x columns, delegate to
// `lib/identity.js` so the `local_credentials` and `identity_links`
// tables are created and backfilled exactly once per installation.
// The two migration steps are independent — `user-model.migrate` can
// run on a DB with NO identity tables yet, and `identity.migrate`
// creates them with FKs pointing at the already-existing `users(id)`.
function migrate(db) {
  db.pragma('journal_mode = WAL');

  // PHA-2704: comment block — keep the legacy `users` schema as the
  // permanent product-identity surface. New code reads/writes password
  // state through `local_credentials` and identity info through
  // `identity_links` (lib/identity.js). The columns below stay as
  // deprecated shadow columns until a future migration drops them.
...
CREATE TABLE IF NOT EXISTS service_health_state (
  ...
  //
  // `user_modulesExistedBefore` records whether this is the very first
  // time this migration has ever run against this DB. It gates the
  // one-time grandfather backfill below (PHA-2200 review finding,
  // comment fef13f26 on PHA-2202): existing users at migration time
  // get all six modules (they already had access to everything), but
  // any user created AFTER that point must only get the registry's
  // DEFAULT_ENABLED set ('wall'), never the full six, on every
  // subsequent boot.
  const userModulesExistedBefore = !!db.prepare(
    "SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'user_modules'"
  ).get();
  // PHA-2659: the CHECK list is now DERIVED from the registry rather
  // than typed out. The original hardcoded six-key literal was exactly
  // the drift this CHECK was meant to prevent — adding a module to
  // lib/modules.js left the DB rejecting the new key until someone
  // remembered to edit this string too. Deriving it makes the registry
  // the single source of truth for both layers.
  //
  // Keys are registry identifiers from a frozen source literal, so
  // interpolating them into DDL is safe; the guard below is
  // belt-and-braces so a future third-party manifest merge can never
  // smuggle a quote into the schema.
  const badKey = modules.MODULE_KEYS.find(k => !/^[a-z0-9_]+$/.test(k));
  if (badKey) {
    throw new Error(`unsafe module key for user_modules CHECK: ${JSON.stringify(badKey)}`);
  }
  const checkList = modules.MODULE_KEYS.map(k => `'${k}'`).join(',');
  db.exec(`
CREATE TABLE IF NOT EXISTS user_modules (
  user_id     INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  module_key  TEXT    NOT NULL CHECK (module_key IN (${checkList})),
  enabled_at  TEXT,
...
`);
  ...
  // PHA-2659: rebuild an EXISTING user_modules whose CHECK predates a
  // registry addition. SQLite cannot ALTER a CHECK constraint in place,
  // so widening the whitelist on a live DB needs the standard table
  // rebuild: new table with the current CHECK, copy rows, drop, rename,
  // recreate the index. (The CREATE above is a no-op on such a DB —
  // IF NOT EXISTS does not reconcile a changed definition.)
  //
  // Detection compares the key set inside the stored `module_key IN
  // (...)` clause against the registry, so this is idempotent — a DB
  // already at the current registry rebuilds nothing — and it repairs
  // drift in either direction.
  //
  // `foreign_keys` must be OFF across the DROP so the users(id) FK
  // isn't enforced against the momentarily-absent table. Homestead's
  // better-sqlite3 build defaults that pragma ON (unlike stock SQLite),
  // so it genuinely needs toggling, and SQLite ignores pragma changes
  // inside a transaction — hence the toggle outside the transaction.
  const existingDDL = db.prepare(
    "SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'user_modules'"
  ).get();
  if (existingDDL && existingDDL.sql) {
    const clause = /module_key\s+IN\s*\(([^)]*)\)/i.exec(existingDDL.sql);
    const storedKeys = clause
      ? (clause[1].match(/'([^']*)'/g) || []).map(s => s.slice(1, -1))
      : [];
    const wanted = modules.MODULE_KEYS;
    const drifted = storedKeys.length !== wanted.length
      || wanted.some(k => !storedKeys.includes(k));
    if (drifted) {
      const fkWasOn = db.pragma('foreign_keys', { simple: true }) === 1;
      if (fkWasOn) db.pragma('foreign_keys = OFF');
      try {
        db.transaction(() => {
          // Rows whose key has LEFT the registry are dropped rather
          // than copied — they would violate the new CHECK, and
          // getUserModules() already ignores unknown keys, so such a
          // row was inert before this ran too.
          db.exec(`
CREATE TABLE user_modules__pha2659 (
  user_id     INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  module_key  TEXT    NOT NULL CHECK (module_key IN (${checkList})),
  enabled_at  TEXT,
  PRIMARY KEY (user_id, module_key)
);
INSERT INTO user_modules__pha2659 (user_id, module_key, enabled_at)
SELECT user_id, module_key, enabled_at FROM user_modules
 WHERE module_key IN (${checkList});
DROP TABLE user_modules;
ALTER TABLE user_modules__pha2659 RENAME TO user_modules;
CREATE INDEX IF NOT EXISTS idx_user_modules_user ON user_modules(user_id);
`);
        })();
      } finally {
      ...
      console.log(`[migrate] user_modules CHECK rebuilt for registry keys: ${wanted.join(', ')}`);
    }
  }
  ...
  if (db.prepare('SELECT COUNT(*) c FROM services').get().c === 0) {
    ...
    ins.run('Example', '/placeholder-app.html', '🔗', 'Replace with your own services', 1);
  }

  // v0.3.0 (PHA-2202) backfill, phase 2, corrected per the PHA-2200
  // review finding (comment fef13f26 on PHA-2202): the original version
  // ran the full six-module grant unconditionally on EVERY boot, which
  // silently gave all six modules to any user created after this
  // migration shipped — contradicting the design note's §2 / Amendment 2
  // / acceptance criterion 2, all of which require brand-new accounts
  // to start with 'wall' only.
  //
  // Split into two passes:
  //   1. One-time grandfather (gated on `userModulesExistedBefore` being
  //      false, i.e. this is the first boot this migration has ever
  //      run): every user that exists RIGHT NOW — including ones just
  //      seeded above on a fresh install — gets all six modules, since
  //      they already had access to everything pre-modularity. This
  //      pass never runs again once the table exists.
  //   2. Ongoing new-user default (runs every boot): any user with ZERO
  //      rows in user_modules — i.e. created after the grandfather pass
  //      already ran — gets only the registry's DEFAULT_ENABLED set.
  //      INSERT OR IGNORE keeps both passes idempotent and never
  //      clobbers a user-toggled row.
  //
...
`);
  ...
  const defaultEnabled = modules.getDefaultEnabled();
  if (defaultEnabled.length > 0) {
    db.prepare(`
INSERT OR IGNORE INTO user_modules (user_id, module_key, enabled_at)
SELECT u.id, d.module_key, datetime('now')
  FROM users u
  JOIN (${defaultEnabled.map(() => "SELECT ? AS module_key").join(' UNION ALL ')}) AS d
  WHERE u.id NOT IN (SELECT DISTINCT user_id FROM user_modules)
`).run(...defaultEnabled);
  }

  // PHA-2704 (identity foundation): delegate to lib/identity.js for the
  // canonical local_credentials + identity_links table creation and
  // one-time backfill. Idempotent on every boot. The call sits last so
  // users(id) is guaranteed to exist before the FK is created.
  require('./identity').migrate(db);
}
...
// `provisionOrClaim` is the canonical CREATE-or-CLAIM path for an
// EXTERNAL identity arriving at Homestead (e.g. an Authentik header).
//   * PHA-2704 (identity foundation): the canonical link mechanism is
//     `identity_links(provider, issuer, provider_subject) → users.id`.
//     Username is no longer the long-term link — it's a transitional
//     fallback for the bootstrap data we just backfilled above.
//   * Match path (in order):
//       1. identity_links lookup by (provider, issuer, provider_subject)
//       2. username COLLATE NOCASE match (transitional; preserved so
//          pre-PHA-2704 Authentik logins don't get re-issued new rows)
//   * row exists → CLAIM (refresh last_seen_at, reconcile groups;
//     history stays). Also link the new (provider, issuer, subject)
//     to the user if no link exists yet — this is the path that
//     upgrades a "username-only" profile to a proper identity_links
//     row, after which future lookups hit branch (1) without the
//     username fallback.
//   * no row → CREATE with display=username, color from palette,
//     auth_provider=<provider>, claimed_at=now, AND link the new
//     identity in the same transaction.
//
// Returns the public-safe user row (no pass_hash, no
// auth_provider/provider_subject — those are now on identity_links).
function provisionOrClaim(db, username, provider, providerSubject, groups) {
  ...
  // Step 1: identity_links match (the canonical PHA-2704 lookup).
  const linked = identity.findUserByIdentityLink(db, providerName, 'legacy-bootstrap', subject);
  let user;
  if (linked) {
    user = db.prepare('SELECT * FROM users WHERE id = ?').get(linked.userId);
    ...
    return finalizeProvision(db, user, groups);
  }

  // Step 2: username COLLATE NOCASE match (transitional bootstrap).
  ...
  if (user) {
    db.prepare(`UPDATE users SET
        ...
        updated_at = datetime('now')
      WHERE id = ?`).run(providerName, subject, user.id);
    // Upgrade: also create the identity_links row so future lookups
    // skip the username fallback. linkIdentity is a no-op if the
    // exact (provider, issuer, subject) already points at this user.
    try {
      identity.linkIdentity(db, user.id, providerName, 'legacy-bootstrap', subject);
    } catch (e) {
      // Collision with a different user: the username row wins for
      // THIS request (legacy compat), but we deliberately don't link
      // the new identity to it. The collision sits in identity_links
      // for admin review — PHA-2706 / the admin recovery flow will
      // resolve it. Swallow the collision here so the request still
      // succeeds; log it for the audit trail.
      console.warn(`[identity] provisionOrClaim collision for ${providerName}/${subject} vs user ${user.id}: ${e.message}`);
    }
    return finalizeProvision(db, user, groups);
  }

  // Step 3: CREATE the user + link the identity in one transaction.
  const tx = db.transaction(() => {
    ...
    identity.linkIdentity(db, r.lastInsertRowid, providerName, 'legacy-bootstrap', subject);
    return r.lastInsertRowid;
  });
  const newId = tx();
...
function finalizeProvision(db, user, groups) {
