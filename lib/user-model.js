...
`);
  ...
  // v0.3.0 (PHA-2220) — Enable/disable event instrumentation.
  //
  // module_events is the raw event log: one row per successful
  // enable/disable call. user_signals is the daily-rollup denormalized
  // shape that downstream analytics queries (and the cohort cron)
  // can read without scanning the raw log.
  //
  // Both are created idempotently so a fresh install and an existing
  // install converge on the same shape. The CHECK constraint on
  // module_events.action is the only validator — there's no need to
  // re-validate module_key against the registry here because the
  // write site (enableModule/disableModule) already validated it via
  // modules.isModuleKey() and threw before reaching the INSERT.
  db.exec(`
...
CREATE INDEX IF NOT EXISTS idx_module_events_user_ts ON module_events(user_id, ts);
CREATE INDEX IF NOT EXISTS idx_module_events_ts     ON module_events(ts);

CREATE TABLE IF NOT EXISTS user_signals (
  user_id                  INTEGER PRIMARY KEY REFERENCES users(id) ON DELETE CASCADE,
  first_module_key         TEXT,
  first_module_enabled_at  TEXT,
  second_module_key        TEXT,
  second_module_enabled_at TEXT,
  total_modules_enabled    INTEGER NOT NULL DEFAULT 0,
...
function enableModule(db, userId, moduleKey, { withRequirements = false } = {}) {
  ...
  // PHA-2220 — capture pre-write enabled state for the target key AND
  // each cascade key so module_events only records real state
  // transitions, not idempotent re-enables. A NULL row means the
  // module is currently disabled; a non-NULL row means it's already
  // enabled (idempotent no-op, no event emitted).
  const preRow = db.prepare(
    'SELECT enabled_at FROM user_modules WHERE user_id = ? AND module_key = ?'
  ).get(userId, moduleKey);
  const targetWasDisabled = !preRow || preRow.enabled_at === null;
  let cascadeWasDisabled = [];
  if (withRequirements) {
    cascadeWasDisabled = needed.map(req => {
      const r = db.prepare(
        'SELECT enabled_at FROM user_modules WHERE user_id = ? AND module_key = ?'
      ).get(userId, req);
      return { key: req, wasDisabled: !r || r.enabled_at === null };
    });
  }
  const tx = db.transaction(() => {
    ...
    db.prepare(`INSERT INTO user_modules (user_id, module_key, enabled_at)
                ...
                ON CONFLICT(user_id, module_key) DO UPDATE SET enabled_at = excluded.enabled_at`)
      .run(userId, moduleKey);
    // PHA-2220 — record the toggle in module_events. The INSERT sits
    // inside the transaction so a thrown error elsewhere rolls back
    // both writes together. We only emit an event when the call
    // represents a real state transition (disabled → enabled).
    // Idempotent re-enables do not pollute the event log. Cascade
    // keys that were already enabled likewise emit no event.
    if (targetWasDisabled) {
      db.prepare(`INSERT INTO module_events (user_id, module_key, action)
                  VALUES (?, ?, 'enable')`)
        .run(userId, moduleKey);
    }
    if (withRequirements) {
      for (const { key, wasDisabled } of cascadeWasDisabled) {
        if (wasDisabled) {
          db.prepare(`INSERT INTO module_events (user_id, module_key, action)
                      VALUES (?, ?, 'enable')`)
            .run(userId, key);
...
function disableModule(db, userId, moduleKey, { withDependents = false } = {}) {
  ...
  // PHA-2220 — capture pre-write enabled state for the target key so
  // module_events only records real transitions (enabled → disabled),
  // not idempotent re-disables. When cascading, we also collect
  // pre-states for the cascade keys.
  const preTarget = db.prepare(
    'SELECT enabled_at FROM user_modules WHERE user_id = ? AND module_key = ?'
  ).get(userId, moduleKey);
  const targetWasEnabled = !!(preTarget && preTarget.enabled_at !== null);
  let cascadeWasEnabled = [];
  if (withDependents) {
    cascadeWasEnabled = dependents.map(dep => {
      const r = db.prepare(
        'SELECT enabled_at FROM user_modules WHERE user_id = ? AND module_key = ?'
      ).get(userId, dep);
      return { key: dep, wasEnabled: !!(r && r.enabled_at !== null) };
    });
  }
  const tx = db.transaction(() => {
    ...
    db.prepare(`INSERT INTO user_modules (user_id, module_key, enabled_at)
                ...
                ON CONFLICT(user_id, module_key) DO UPDATE SET enabled_at = NULL`)
      .run(userId, moduleKey);
    // PHA-2220 — record the toggle(s) in module_events. Idempotent
    // re-disables (already-NULL) emit no event. Cascade disables
    // emit one event per cascade key that actually transitioned.
    if (targetWasEnabled) {
      db.prepare(`INSERT INTO module_events (user_id, module_key, action)
                  VALUES (?, ?, 'disable')`)
        .run(userId, moduleKey);
    ...
    if (withDependents) {
      for (const { key, wasEnabled } of cascadeWasEnabled) {
        if (wasEnabled) {
          db.prepare(`INSERT INTO module_events (user_id, module_key, action)
                      VALUES (?, ?, 'disable')`)
            .run(userId, key);
...
// `summarizeModuleEvents` (PHA-2220) — recompute the user_signals
// denormalized rollup from module_events. Idempotent: callers can
// re-run after each event without coordination. The rollup captures:
//   * first_module_key / first_module_enabled_at — earliest enable event
//     for the user (NULL if no enable events yet, e.g. seeded default).
//   * second_module_key / second_module_enabled_at — second distinct
//     enable event, by ts. This is the "second module enabled" signal
//     the cohort summary was created to surface.
//   * total_modules_enabled — count of distinct modules with at least
//     one enable event in the user's history (current enabled state is
//     not the same — a user could have toggled off; we count *enables*
//     because the product question is "what did they turn on").
//
// All writes happen inside one transaction so a partial failure rolls
// back cleanly. Returns the number of user_signals rows written.
function summarizeModuleEvents(db) {
  const tx = db.transaction(() => {
    // Per-user aggregate: earliest + second-earliest enable events
    // by ts, plus the count of distinct modules the user has ever
    // enabled (regardless of whether they're currently enabled —
    // disable events do not subtract from the count). The total is
    // what the product question "what modules did they turn on" is
    // asking about, not the current enabled set.
    db.exec(`
INSERT OR REPLACE INTO user_signals (
  user_id, first_module_key, first_module_enabled_at,
  second_module_key, second_module_enabled_at,
  total_modules_enabled, updated_at
)
SELECT
  per_user.user_id,
  MIN(CASE WHEN rn = 1 THEN module_key END) AS first_module_key,
  MIN(CASE WHEN rn = 1 THEN ts END) AS first_module_enabled_at,
  MIN(CASE WHEN rn = 2 THEN module_key END) AS second_module_key,
  MIN(CASE WHEN rn = 2 THEN ts END) AS second_module_enabled_at,
  (SELECT COUNT(DISTINCT me2.module_key)
     FROM module_events me2
     WHERE me2.user_id = per_user.user_id
       AND me2.action = 'enable') AS total_modules_enabled,
  datetime('now') AS updated_at
FROM (
  SELECT
    user_id, module_key, ts,
    ROW_NUMBER() OVER (
...
`);
  ...
  tx();
  // We only write user_signals rows for users who actually have at
  // least one enable event. Users on default-enabled wall only do
  // not appear in user_signals — getUserSignals returns null for
  // them, which is a true answer (no enable events = no signal).
  return db.prepare('SELECT COUNT(*) c FROM user_signals').get().c;
}

// `getUserSignals` (PHA-2220) — read the denormalized rollup for one
// user. Returns null when no enable events exist (the user has only
// default-enabled modules). Useful for the SPA / API to surface the
// cohort signal without scanning module_events directly.
function getUserSignals(db, userId) {
  return db.prepare(
