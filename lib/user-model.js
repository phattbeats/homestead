// Homestead user-model data layer (PHA-1618, v0.0.5).
//
...
// ---- tile visibility helpers (PHA-1863, v0.1.2) ----
//
// Tile visibility is split across two tables, with the same
// (tile_kind, tile_id) composite key:
//   * tile_visibility_groups(tile_kind, tile_id, group_id) — broad access
//   * tile_visibility_users(tile_kind, tile_id, user_id)   — per-user override
//
// A tile is "visible to a user" iff ANY of:
//   (a) the tile has zero rows in BOTH tables (open to everyone)
//   (b) the user is in a group row of tile_visibility_groups for this tile
//   (c) the user has a row in tile_visibility_users for this tile
//
// The `?visibility=` query param is a UI filter layered on top:
//   * 'mine'   (default) — only tiles owned by the authenticated user
//   * 'shared' — only tiles the user has an explicit visibility row on
//   * 'all'    — every tile the user can see under the base rule
//
// Admin-only writes (POST/PUT enforce via the route; the helpers below
// are pure DB).

const TILE_KIND_SERVICE = 'service';

function isTileVisible(db, userId, tileKind, tileId) {
  const counts = db.prepare(`
    SELECT
      (SELECT COUNT(*) FROM tile_visibility_groups WHERE tile_kind = ? AND tile_id = ?) AS g,
      (SELECT COUNT(*) FROM tile_visibility_users  WHERE tile_kind = ? AND tile_id = ?) AS u
  `).get(tileKind, tileId, tileKind, tileId);
  if (counts.g === 0 && counts.u === 0) return true; // open to everyone
  const groupHit = db.prepare(`
    SELECT 1 FROM tile_visibility_groups tvg
    JOIN user_groups ug ON ug.group_id = tvg.group_id
    WHERE tvg.tile_kind = ? AND tvg.tile_id = ? AND ug.user_id = ?
    LIMIT 1
  `).get(tileKind, tileId, userId);
  if (groupHit) return true;
  const userHit = db.prepare(`
    SELECT 1 FROM tile_visibility_users
    WHERE tile_kind = ? AND tile_id = ? AND user_id = ?
    LIMIT 1
  `).get(tileKind, tileId, userId);
  return !!userHit;
}

function getTileVisibility(db, tileKind, tileId) {
  const groups = db.prepare(`
    SELECT g.name FROM tile_visibility_groups tvg
    JOIN groups g ON g.id = tvg.group_id
    WHERE tvg.tile_kind = ? AND tvg.tile_id = ?
    ORDER BY g.name
  `).all(tileKind, tileId).map(r => r.name);
  const users = db.prepare(`
    SELECT u.username FROM tile_visibility_users tvu
    JOIN users u ON u.id = tvu.user_id
    WHERE tvu.tile_kind = ? AND tvu.tile_id = ?
    ORDER BY u.username
  `).all(tileKind, tileId).map(r => r.username);
  return { groups, users };
}

function setTileVisibility(db, tileKind, tileId, groupNames, usernames) {
  // Clear-then-write semantics. Unknown group names are normalized via
  // getOrCreateGroupId (auto-creates the row in `groups`). Unknown
  // usernames resolve via case-insensitive lookup; missing rows are
  // silently dropped so a stale PUT that references a deleted user
  // doesn't 500.
  const cleanNames = (arr) => (Array.isArray(arr) ? arr : [])
    .map(n => (n || '').toLowerCase().trim())
    .filter(n => /^[a-z0-9_-]{2,32}$/.test(n));
  const validGroups = cleanNames(groupNames);
  const validUsers = cleanNames(usernames);

  const groupIds = validGroups.map(n => getOrCreateGroupId(db, n)).filter(Boolean);
  const userIds = validUsers
    .map(n => db.prepare('SELECT id FROM users WHERE username = ?').get(n))
    .filter(Boolean)
    .map(r => r.id);

  const tx = db.transaction(() => {
    db.prepare('DELETE FROM tile_visibility_groups WHERE tile_kind = ? AND tile_id = ?').run(tileKind, tileId);
    const insGroup = db.prepare('INSERT OR IGNORE INTO tile_visibility_groups (tile_kind, tile_id, group_id) VALUES (?, ?, ?)');
    for (const gid of groupIds) insGroup.run(tileKind, tileId, gid);
    db.prepare('DELETE FROM tile_visibility_users WHERE tile_kind = ? AND tile_id = ?').run(tileKind, tileId);
    const insUser = db.prepare('INSERT OR IGNORE INTO tile_visibility_users (tile_kind, tile_id, user_id) VALUES (?, ?, ?)');
    for (const uid of userIds) insUser.run(tileKind, tileId, uid);
  });
  tx();
}

// `getServicesForUser` returns the service rows the requester can see
// under the chosen `?visibility=` mode. The session-side visibility
// filter (open-to-all / group / user override) is applied only in
// `all` mode — `mine` and `shared` are explicit user-chosen axes.
function getServicesForUser(db, userId, username, visibility) {
  const mode = ['mine', 'shared', 'all'].includes(visibility) ? visibility : 'mine';
  if (mode === 'mine') {
    return db.prepare('SELECT * FROM services WHERE owner = ? ORDER BY sort, id').all(username);
  }
  if (mode === 'shared') {
    return db.prepare(`
      ...
      WHERE
        EXISTS (
          SELECT 1 FROM tile_visibility_groups tvg
          JOIN user_groups ug ON ug.group_id = tvg.group_id
          WHERE tvg.tile_kind = 'service' AND tvg.tile_id = s.id AND ug.user_id = ?
        )
        OR EXISTS (
          SELECT 1 FROM tile_visibility_users tvu
          WHERE tvu.tile_kind = 'service' AND tvu.tile_id = s.id AND tvu.user_id = ?
        )
      ORDER BY s.sort, s.id
    `).all(userId, userId);
  }
  // 'all' — apply the base visibility filter (open OR group OR user)
  return db.prepare(`
    SELECT s.* FROM services s
    WHERE
      (NOT EXISTS (SELECT 1 FROM tile_visibility_groups tvg
                    WHERE tvg.tile_kind = 'service' AND tvg.tile_id = s.id)
       AND NOT EXISTS (SELECT 1 FROM tile_visibility_users tvu
                       WHERE tvu.tile_kind = 'service' AND tvu.tile_id = s.id))
      OR EXISTS (
        SELECT 1 FROM tile_visibility_groups tvg
        JOIN user_groups ug ON ug.group_id = tvg.group_id
        WHERE tvg.tile_kind = 'service' AND tvg.tile_id = s.id AND ug.user_id = ?
