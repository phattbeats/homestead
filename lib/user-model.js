...
// `isAgentEnabled` (PHA-2208 / PHA-2200.7): returns true when the user
// has the `agent` module in their enabled set. The agent module gates
// the meta-agent surface (chat drawer FAB + outbound `/api/drawer`
// POST + the forthcoming `/api/gazette/brief` morning-brief endpoint,
// per Brandon's PHA-1617 design note §3). Centralised here so the
// route-entry middleware and the SPA bootstrap read the same source.
//
// Implementation: thin wrapper over `getEnabledModules` so the
// registry-driven filtering (PHA-2203 Amendment 1) is the same logic
// the SPA uses. Unknown / legacy keys are silently skipped by
// `getEnabledModules`, so this helper never reports a module that
// isn't in the live registry.
function isAgentEnabled(db, userId) {
  return getEnabledModules(db, userId).some(e => e.key === 'agent');
}
