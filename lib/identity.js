...
// fail. PHA-2708 hard-codes the owner concept to `is_admin = 1`
// (which the user-model reconcile keeps in sync with the `admins`
// Authentik group). If a future PHA introduces multi-admin, this
// constant + `findOwnerUserId` below are the two places to revisit.
...
//
// PHA-2708 strengthening: for the household owner, refuses the unlink
// when this removal would leave the owner with no path back into
// Homestead (no local credential AND this was the last identity_link).
// The block reason `would_lock_out_owner` is distinct from
// `no_login_path` so callers can surface a clear "this is the owner"
// error. Returns { removed: true|false, blocked: 'no_login_path' |
// 'would_lock_out_owner' | null }.
...
// ---- PHA-2708 owner recovery primitives ----
//
...
// the LAN password.
//
// `parseIsoUtc` — robust ISO-8601 → epoch-ms parser. The values we
// read back from `local_credentials.recovery_token_expires_at` come
// from two writers: `mintOwnerRecoveryToken` stores strings that
// already end in `Z` (from `Date#toISOString`), and the original
// `datetime('now')` SQLite default is `YYYY-MM-DD HH:MM:SS` with no
// timezone. Naive concatenation (`new Date(s + 'Z')`) returns NaN
// on the SQLite-default shape AND double-Zs the stored ISO value.
// Treat the value as UTC no matter what: append `Z` if missing,
// trim a trailing `Z` if it's already there, then parse.
//
...
// Storage: `local_credentials.recovery_token_hash` already exists in
// the PHA-2704 schema (sha256(token) → stored; plaintext never
// persisted) with `recovery_token_expires_at`. This section adds the
// mint + consume primitives on top of those columns, plus an
// `auditOwnerRecovery` helper that records every event into the
// analytics_events table so the operator has a recoverable trail
...
//     (sha256, hex) before persisting to `recovery_token_hash`.
...
function mintOwnerRecoveryToken(db, { ttlMs = 60 * 60 * 1000 } = {}) {
  ...
  const existing = db.prepare(`
    SELECT recovery_token_hash AS h, recovery_token_expires_at AS exp
  ...
  db.prepare(`
    INSERT INTO local_credentials (user_id, recovery_token_hash, recovery_token_expires_at, updated_at)
    VALUES (?, ?, ?, datetime('now'))
    ON CONFLICT(user_id) DO UPDATE SET
...
function consumeOwnerRecoveryToken(db, token, newPassword) {
  ...
  const row = db.prepare(`
    SELECT recovery_token_hash AS h, recovery_token_expires_at AS exp, password_hash AS pw
      FROM local_credentials WHERE user_id = ?
  `).get(ownerId);
  ...
  if (parseExpiresAt(row.exp) <= Date.now()) {
    ...
    db.prepare('UPDATE local_credentials SET recovery_token_hash = NULL, recovery_token_expires_at = NULL, updated_at = datetime(\'now\') WHERE user_id = ?').run(ownerId);
    return { ok: false, code: 'invalid_or_expired_token' };
  }
