...
CREATE TABLE IF NOT EXISTS agent_connection_replays (
  request_id TEXT PRIMARY KEY,
  connection_id INTEGER NOT NULL REFERENCES agent_connections(id) ON DELETE CASCADE,
  seen_at TEXT NOT NULL DEFAULT (datetime('now'))
);
CREATE INDEX IF NOT EXISTS idx_agent_connection_replays_seen_at
  ON agent_connection_replays(seen_at);
CREATE INDEX IF NOT EXISTS idx_agent_connection_replays_connection
  ON agent_connection_replays(connection_id);
`);
}

// PHA-3199: charset for X-Homestead-Request-Id. Keep it boring — UUIDs,
// short opaque tokens, dot/dash prefixes are all that's needed. Anything
// else is junk we don't want sitting in the replay ledger.
const REQUEST_ID_PATTERN = /^[A-Za-z0-9._-]+$/;
const REQUEST_ID_MAX_LEN = 128;

// Validates an inbound X-Homestead-Request-Id. Returns the trimmed id
// on success, or null if it's missing/empty/oversized/has characters
// outside the allowed charset. Callers should treat null as a 401 —
// we do NOT sign-verify against an attacker-controlled "request id"
...
// PHA-3199: replay-window (5 min skew + 1 min slack, in seconds). The
// skew check in verifySignature() is the strict gate; this window is
// only used by the cheap-purge path so the table doesn't grow forever.
const REPLAY_WINDOW_SECONDS = 6 * 60; // 5 min skew + 1 min slack

// PHA-3199: best-effort garbage collection on the replay table. Cheap
// DELETE; the table is small (one row per accepted event in the last
// 6 minutes), so a per-write purge is fine. The delete is wrapped in
// try/catch because callers invoke it as a "should not break the
// request" housekeeping step; if the DB is wedged for any reason we'd
// rather reject nothing than 500 a real replay rejection.
function purgeOldReplays(db, { nowSeconds = Math.floor(Date.now() / 1000) } = {}) {
  try {
    db.prepare(`DELETE FROM agent_connection_replays
       WHERE seen_at < datetime('now', '-${REPLAY_WINDOW_SECONDS} seconds')`).run();
  } catch (_) { /* swallow — replay table is best-effort housekeeping */ }
}

// PHA-3199: records a freshly accepted request id. Returns true if the
// row was inserted (first time we've seen this id); false if it
// already existed (replay). Callers treat `false` as 409 — do not
// call mailbox.postMessage on a duplicate.
//
// better-sqlite3 throws SqliteError on UNIQUE-violation rather than
// returning changes=0, so we have to catch and inspect — the duplicate
// path is success-from-our-POV ("yes, we detected the replay"), not a
// failure to log. Any other error is re-thrown.
function recordReplay(db, { connectionId, requestId }) {
  try {
    const r = db.prepare(`INSERT INTO agent_connection_replays
         (request_id, connection_id) VALUES (?, ?)`).run(requestId, connectionId);
    return r.changes === 1;
  } catch (err) {
