...
// Lightweight aggregate for a (userId, appId) pair — the "activity
// summary" GET /api/apps/:key (PHA-2229) inlines alongside the app's
// metadata, as opposed to the full paginated list() above.
function summary(db, userId, appId) {
  const row = db.prepare(
    `SELECT COUNT(*) AS call_count, MAX(created_at) AS last_used_at
       FROM app_api_log WHERE user_id = ? AND app_id = ?`
  ).get(userId, appId);
  return { call_count: row.call_count, last_used_at: row.last_used_at };
}

module.exports = { migrate, log, list, summary };
