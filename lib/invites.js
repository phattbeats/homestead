...
function redeem(db, code, userId) {
  ...
  const tx = db.transaction(() => {
    // PHA-2766: in-tx capacity re-check. `peek()` above already
    // returned ok, but in a concurrent race a second request could
    // bump uses_count past max_uses between our peek and our INSERT.
    // Without this recheck the existing `redeem()` would silently
    // over-redeem: peek sees uses_count=24 (cap=25), both requests
    // pass the peek gate, both INSERT a canary row and both UPDATE
    // uses_count to 26 — exceeding the 25-use budget the issue body
    // promised. The recheck under the write tx catches that race: the
    // second tx sees uses_count=25 from the first tx's commit and
    // 410s with `invite_already_redeemed`, matching the same
    // peek-then-INSERT semantics that `signupViaInvite` already uses
    // for the closed-beta vertical (lines below). Single-use
    // (max_uses=1) is unchanged in observable behavior — the same
    // `peek()` 410 already gates it before reaching the tx.
