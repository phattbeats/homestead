...
/* PHA-2835: scrubPublicNote — the public peek at
 *   GET /api/public/invites/:code
 * is unauthenticated, so any metadata an admin typed into the invite
 * `note` field lands in the hands of a fresh invitee. Admin notes are
 * a private scratchpad ("for the Tailor kid"), but operations have
 * historically drifted to writing PHA-#### / wave / reissue copy in
 * the same field, which leaks on the landing page.
 *
 * scrubPublicNote strips operational patterns and returns either a
 * short human-facing note or null. The defense-in-depth pass on the
 * client (public/invite.html `sanitizeNote`) keeps admin copy that
 * sneaks past this gate from rendering.
 */
const PUBLIC_NOTE_BAD_PATTERNS = [
  /\bPHA-\d{2,}(?:\.\d+)?\b/gi,    // PHA-1234, PHA-1234.5
  /\b(beta\s*wave)\b/gi,
  /\b(re-?issued?|re-?issue)\b/gi,
  /\b(wiped?|wipe)\b/gi,
  /\b(internal|staff|ops)\s+(note|copy|reset)/gi,
  /\b(admin|owner)\s+(note|reset|recovery)/gi,
  /\bafter\s+PHA-\d+/gi,             // "after PHA-2728"
  /\b(reset\s+event)\b/gi,           // "reset event"
];
const PUBLIC_NOTE_MAX_LENGTH = 280;

function scrubPublicNote(rawNote) {
  if (!rawNote || typeof rawNote !== 'string') return null;
  const trimmed = rawNote.trim();
  if (trimmed.length === 0) return null;
  if (trimmed.length > PUBLIC_NOTE_MAX_LENGTH) return null;
  let scrubbed = trimmed;
  for (const re of PUBLIC_NOTE_BAD_PATTERNS) scrubbed = scrubbed.replace(re, '');
  // Drop orphaned whitespace + trailing separators left by removals.
  // "reset after " → "reset after", "admin note:" → "admin note",
  // "(25-use porch invite, after )" → "(25-use porch invite, after)".
  scrubbed = scrubbed.replace(/\s{2,}/g, ' ').trim();
  scrubbed = scrubbed.replace(/\s+([,;:.)\]])/g, '$1').trim();
  scrubbed = scrubbed.replace(/([,;:.)\]])\s*$/, '').trim();
  scrubbed = scrubbed.replace(/^[(,;:.]\s*/, '').trim();
  if (scrubbed.length === 0) return null;
  // If scrubbing removed most of the original, the note was almost
  // certainly operational copy with a thin human wrapper. Drop it
  // rather than risk leaking the wrapper.
